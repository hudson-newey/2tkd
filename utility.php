...
while ($represents->fetch()) {
	$represents_display_lookup[$represents->represents_id] = stripslashes($represents->name);	
}
	...
	$smarty->assign('active_tournament', array(
	      ...
	      'SCHEDULE_HTML'							=> stripslashes($active_tournament->schedule_html),
...
function Get_Event_Max_Competitors($id) {
	
	$events = DB_DataObject::factory('events');		
	$events->event_id = $id;
	if ($events->find()) {
		$events->fetch();
		return $events->max_competitors;	
	} else {
		return " ";	
	}
